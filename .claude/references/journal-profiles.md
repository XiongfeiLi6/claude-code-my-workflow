...
**Focus.** Flagship field journal for environmental and resource economics. Publishes theoretical and empirical work on pollution, climate, energy, natural resources, nonmarket valuation, and environmental policy design. Favors papers where the environmental application is intrinsic to the question, not just a setting for a generic applied-micro exercise.

**Bar.** Field-journal desk bar, not top-5: the question must matter to environmental economists and the credibility must be solid, but a clever-but-narrow design with a real policy payoff clears the desk where it might not at QJE. Papers that are "a DiD that happens to use pollution data" without environmental-economics engagement face skepticism.

**Domain-referee adjustments.**
- Contribution 30 → 25 (field-journal calibration; novelty bar is real but not top-5)
- External validity 15 → 20 (policy transferability matters — does the result inform regulation beyond this setting?)
- Fit 10 → 15 (must genuinely be environmental/resource economics)

**Methods-referee adjustments.**
- Identification 35 → 35 (unchanged — JEEM referees read the modern DiD/IV literature)
- Robustness 15 → 20 (pollution/weather instruments and staggered-adoption designs attract heavy robustness scrutiny)
- For `reduced-form` papers: spatial spillovers / SUTVA violations (pollution transport, leakage, market reallocation) are a mandatory sanity check — treat unaddressed cross-boundary spillovers as a FAIL-level concern.
- For `structural` papers: damage-function and discount-rate calibration must be defended against the literature (e.g., integrated-assessment or valuation parameter choices).
- Measurement: pollution-exposure measurement error (monitor siting, satellite vs. ground data, strategic monitoring) is a first-order concern, not a footnote.

**Typical concerns.**
- "Is the environmental margin intrinsic to the question, or is this a generic applied-micro paper in an environmental setting?"
- "Do the authors address pollution transport / leakage / spatial spillovers that violate SUTVA?"
- "Is exposure measured credibly — monitor coverage, siting endogeneity, satellite validation?"
- "Does the instrument (weather, inversions, wind direction) plausibly satisfy exclusion in *this* outcome equation?"
- "What is the policy counterfactual, and are welfare/magnitude claims calibrated to it?"
...
**Table format override.** No strict house style; SE in parentheses, clustering level stated in table notes. Elsevier journal — significance stars accepted but state exact conventions in notes.
