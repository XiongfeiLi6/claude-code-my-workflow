...
Symmetric partner to **Pre-Flight Reports** (skill-level `## Phase 0` input checks). Where Pre-Flight proves inputs were read *before* work, Post-Flight proves the output's factual claims hold *after* drafting — before the skill returns to the user. In this referee workflow it enforces Article III (Evidence Integrity): every evidence anchor in a referee report is verified against the manuscript before delivery.
...
- `.claude/rules/constitutional-governance.md` — Article III (Evidence Integrity), which this rule operationalizes.
