...
- Merge surface area with the upstream template repo grows.
...
- Commit the new skill on the project's working branch.
...
The `/learn` skill is not modified by this rule (its body tracks the
upstream template); this rule is consulted by the orchestrator at the
moment `/learn` runs.
...
Archival is a manual step in this project (no `/context-audit` skill is
installed here); this rule defines the threshold (12 months no use).
