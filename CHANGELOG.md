...
## Local fork addendum — `manuscript-writing-style.md` module evolution (2026-05-22 → 2026-06-03)

**Scope:** entries below describe changes to a single path-scoped rule (`.claude/rules/manuscript-writing-style.md`) on the local fork only. They are **not** part of upstream `pedrohcgs/claude-code-my-workflow`'s release versioning. The "v" numbers here are an independent semver thread for the rule module; the upstream release versions (above) are unaffected.

The module evolved through four iterations between 2026-05-22 and 2026-06-03, culminating in a constitutional rewrite. The summary below collapses the four iterations into one entry. Detailed prior-iteration content remains accessible in git history at the commits cited.

### Module — local-mws-v1.9.1 (2026-06-03)

**Status:** Constitutional standard for every empirical and theoretical economics paper the author writes. Calibrated to top-5 best practice. 501 lines.

**Structure:**
- §0 Calibration and scope (minimalist vs discursive register; empirical/theory/hybrid; AE audience model with reading-time table).
- §1 Foundational principle (world-claim vs reader-direction; three derived rules).
- §2 Document architecture (empirical, theory, hybrid section sequences with page weights).
- §3 Section templates (Abstract, Introduction, Related Literature, Institutional Context, Conceptual Framework, Data, Empirical Strategy, Results, Robustness, Discussion/Policy, Conclusion, Appendix).
- §4 Paragraph-level templates (Subsection Opener, Result Paragraph, Mechanism/Channel Paragraph, Caveat, Transition, Footnote).
- §5 Sentence-level conventions (voice/agency, tense, hedging, statistical-significance language, number presentation).
- §6 Visual conventions (table captions/notes, robustness summary, figure captions, equations, emphasis, citation style, cross-references).
- §7.1 The five high-level failure modes (structural).
- §7.2 Specific patterns to grep and cut — tactical audit checklist organized into eight categories (over-defensive expressions; inline statistical reporting; punctuation overuse including em-dash overuse and p-values in parens; restatement patterns; empty intensifiers and transition words; verbose constructions; formatting and visual noise; citation hygiene).
- §8 Over-application failures ("edit the wrapper, keep the core").
- §9 Sanity checks (eight items including inferential-bridge check, promised-robustness check, mechanism-acknowledgement check).

**Iteration history (commits in `my-customizations`):**
- `28c6b10` (2026-05-22): initial path-scoped rule; declarative-not-defensive principle, β/SE in tables, per-SD magnitudes, no result pre-announcement.
- `6c3c2aa` (2026-05-22, same-day rewrite): positive-templates-first structure replaces the initial "anti-pattern catalogue" framing.
- `686ba2d` (2026-06-03): added Introduction architecture template, Abstract inferential-step rule, Robustness summary per-SD column, Policy mechanism acknowledgement, Promised robustness delivery audit, two anti-patterns (parenthetical narrative labels; conclusion-without-bridge).
- `01ecdae` (2026-06-03, constitutional rewrite): promoted from style sheet to constitutional standard; +AE audience model, +document architecture, +12 section templates, +mechanism/channel paragraph, +sentence-level conventions section, +visual conventions section; compressed 14 anti-patterns to 5 high-level failure modes.
- `0cd42f7` (2026-06-03, balance correction): restored the tactical audit specificity that the constitutional rewrite had compressed; added §7.2 with eight tactical sub-categories.

**Net effect:** 117 → 501 lines over four iterations on the same day in early June, after a 2026-05-22 initial seed. The module is now stable; further changes will be patch-only.

**Tested in production:** the rule's section templates and tactical audit were applied to a top-5-target carbon-leakage / Chinese-enforcement paper for the LSE–Yale Firms, Trade, and Development conference (deadline 2026-06-01). The Introduction architecture audit caught failures that would otherwise have shipped; the abstract inferential-bridge rule produced a measurably clearer policy claim; the §7.2 audit caught p-values in parens and em-dash overuse before submission.
