...
[LEARN:workflow] Commit-cost control: the token cost of committing is the ceremony, not `git commit`. Default the commit skill to quick mode (script gates + stage + commit on the current branch, read `git diff --stat`, never full diffs) pinned to the Sonnet tier; reserve verifier spawn + branch→PR→merge for `/commit full` at milestones. Cadence: one commit per completed logical task — never per-edit, never multi-day batches.

[LEARN:workflow] Model routing is declarative, not manual: pin `model:` aliases (haiku/sonnet/opus) in agent and skill frontmatter per `.claude/rules/model-routing.md`; keep the personal session default in gitignored `settings.local.json`. Aliases survive model-generation upgrades; hardcoded model IDs go stale. The main loop's model is the only layer that still needs a manual `/model` switch (top tier for the hardest sessions only).
